...
public class SkillWnd extends Widget {
    ...
    public class SkillGrid extends GridList<Skill> {
        ...
        public SkillGrid(Coord sz) {
            super(sz);
            nsk = new Group(UI.scale(40, 40), new Coord(-1, 5), Resource.getLocString(Resource.BUNDLE_LABEL, "Available Skills"), Collections.emptyList());
            csk = new Group(UI.scale(40, 40), new Coord(-1, 5), Resource.getLocString(Resource.BUNDLE_LABEL, "Known Skills"), Collections.emptyList());
    ...
    public SkillWnd() {
        ...
        prev = add(new Img(catf.render(Resource.getLocString(Resource.BUNDLE_LABEL, "Entries")).tex()), width, 0);
        ...
        Tabs.Tab sktab = lists.add();
        {
            ...
            Button bbtn = sktab.adda(new Button(UI.scale(50), Resource.getLocString(Resource.BUNDLE_BUTTON, "Buy")).action(() -> {
                if (skg.sel != null)
                    skill.wdgmsg("buy", skg.sel.nm);
            }), bf.pos("ibr").subs(10, 0).y(bf.pos("mid").y), 1.0, 0.5);
            Label clbl = sktab.adda(new Label(Resource.getLocString(Resource.BUNDLE_LABEL, "Cost:")), bf.pos("iul").adds(10, 0).y(bf.pos("mid").y), 0, 0.5);
        ...
        addhlp(lists.c.add(0, lists.sz.y + UI.scale(5)), UI.scale(5), lists.sz.x,
                lists.new TabButton(0, Resource.getLocString(Resource.BUNDLE_BUTTON, "Skills"), sktab),
                lists.new TabButton(0, Resource.getLocString(Resource.BUNDLE_BUTTON, "Credos"), credos),
                lists.new TabButton(0, Resource.getLocString(Resource.BUNDLE_BUTTON, "Lore"), exps));
        pack();
    }
